...
{
  trace_puts("Chỉnh tốc độ quạt");
  trace_puts("1. Nhiệt độ > 25 sẽ bật quạt");
  trace_puts("2. Nhiệt độ < 25 sẽ tắt quạt");
  timer_start();
  ...
  while (1)
    {
      ...
      if (temperature[seconds % 5] > 25) {
        blink_led_on(); // Turn on the LED
        trace_puts("BẬT QUẠT, LED XANH BAT!");
      } else {
        blink_led_off(); // Turn off the LED
        trace_puts("TẮT QUẠT, LED XANH TAT!");
      }
    }
}
